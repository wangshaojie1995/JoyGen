...
#
# Modification:
# - Added new methods for more refined control of the mask and latents. 
...
class VAE():
    ...
    def __init__(self, model_path="./models/sd-vae-ft-mse/", resized_img=256, use_float16=False, device="cpu"):
        ...
        self._upper_mask_tensor = self.get_upper_mask_tensor()
    ...
    def get_upper_mask_tensor(self):
        """
        Creates a mask tensor for image processing.
        :return: A mask tensor.
        """
    ...
    def preprocess_img(self,img_name,half_mask=False,upper_half_mask=False):
        """
        ...
        if half_mask and upper_half_mask:
            sys.exit(1)
        if half_mask:
            x = x * (self._mask_tensor>0.5)
        if upper_half_mask:
            x = x * (self._upper_mask_tensor>0.5)
        x = self.transform(x)
    ...
    def decode_latents_for_train(self, latents):
        """
        Decode latent variables back into an image.
        :param latents: The latent variables to decode.
        :return: the decoded image with tensor format for training.
        """
        latents = (1/  self.scaling_factor) * latents
        image = self.vae.decode(latents.to(self.vae.dtype)).sample
        image = (image / 2 + 0.5).clamp(0, 1)
        image = image.permute(0,2,3,1) * 255
        image = torch.round(image.type(torch.uint8))
        image = torch.flip(image, [3]).to(torch.float)
        return image


    # latent for nomask image
    def get_latents_for_nomask(self, img):
        image = self.preprocess_img(img,half_mask=False) # [1, 3, 256, 256] RGB, torch tensor
        nomask_latent = self.encode_latents(image) # [1, 4, 32, 32], torch tensor
        return nomask_latent

    # latent for mask image
    def get_latents_for_mask(self, img):
        image = self.preprocess_img(img,half_mask=True) # [1, 3, 256, 256] RGB, torch tensor
        mask_latent = self.encode_latents(image) # [1, 4, 32, 32], torch tensor
        return mask_latent

    # ground truth latent
    def get_latents_for_gt(self, img):
        gt_image = self.preprocess_img(img,half_mask=False) # [1, 3, 256, 256] RGB, torch tensor
        gt_latent = self.encode_latents(gt_image) # [1, 4, 32, 32], torch tensor
        return gt_latent

    # masked gt latent 
    def get_latents_for_mask_gt(self, img):
        mask_gt_image = self.preprocess_img(img,half_mask=True) # [1, 3, 256, 256] RGB, torch tensor
        mask_gt_latent = self.encode_latents(mask_gt_image) # [1, 4, 32, 32], torch tensor
        return mask_gt_latent


    # reference image latent
    def get_latents_for_ref(self, img):
        ref_image = self.preprocess_img(img,half_mask=False) # [1, 3, 256, 256] RGB, torch tensor
        ref_latent = self.encode_latents(ref_image) # [1, 4, 32, 32], torch tensor
        return ref_latent

    def get_latents_for_multi_reference(self, img, half_mask=False, upper_half_mask=False):
        image = self.preprocess_img(img, half_mask=half_mask, upper_half_mask=upper_half_mask)
        latent = self.encode_latents(image)
        return latent

    # input latent for unet, while predicted latent will be compared with gt latent
    def get_latents_for_unet(self,img):
        ...
        return latent_model_input, ref_image
